...
Changes vs V3.2:
  - _build_step1_prompt() now accepts an optional security_profile arg.
    When set to "basic" or "enterprise", get_topology_security_prompt()
    appends the appropriate security architecture rules to the prompt.
  - generate_network_topology() forwards the profile's security_profile.
  - process_and_save_topology() unchanged.
 
All other logic (port assigner, hardware injection, VLAN patching,
Pydantic validation, retry loop) is identical to V3.2.
...
from preflight import PreflightProfile                          # NEW import
from schema import (
    GNS3Project, TopologyRequest, validate_topology_request, validate_topology,
)
from security_prompts import get_topology_security_prompt      # NEW import
...
def _call_step1(
    ...
    client = _get_client()
    prompt = _build_step1_prompt(devices, security_profile=security_profile)
...
def generate_network_topology(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        topo_request = _call_step1(
            user_request, devices,
...
def process_and_save_topology(
    ...
    Unchanged from V3.2 — security profile does not affect hardware
    injection or VLAN patching.
    """
